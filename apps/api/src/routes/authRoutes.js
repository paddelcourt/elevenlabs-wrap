...
router.get('/callback', async (req, res) => {
  ...
  if (!code) {
    return res.redirect(`${authService.frontendUrl}/#login?error=no_code`);
  }
  ...
  try {
    ...
    req.session.save((err) => {
      if (err) {
        ...
        return res.redirect(`${authService.frontendUrl}/#login?error=session_save_failed`);
      }

      // Redirect to specific page or default (with hash route)
      if (redirectPage) {
        res.redirect(`${authService.frontendUrl}/${redirectPage}.html?success=true`);
      } else {
        res.redirect(`${authService.frontendUrl}/#login?success=true`);
      }
    });
  } catch (error) {
    ...
    if (redirectPage) {
      ...
      res.redirect(`${authService.frontendUrl}/#login?error=auth_failed`);
    }
  }
});
